...
string transBinaryStrToASCII(string bStr){
    ...
    for(int i=0;i<bStr.length()/8;i++){
        ...
        if(t==32) continue; //把0x20跳过，因为名字用0x20填充
        //cout<<"t="<<t<<" ";
        answer+=(char )t;
    }
    //cout<<endl;
...
/**
 * 遍历文件链表（若发现子目录，则转过去给traverseDirectory
...
void traverseDirectoryNode(DirNode* father,string entries){
    for(int i=0;i<entries.length()/(32*8);i++){
        ...
        if(entry.substr(0,8)=="11100101") continue;//以这个开头的是废弃的entry
...
/**
 * 根据提示的绝对地址或文件名字找文件（注意:暂时先设定必须给出完整地址）
 * @param absoluteLocation
 * @return
 */
File* findFileByLoc(string absoluteLocation){
    string newAbsoluteLocation="/";
    if(absoluteLocation=="") return NULL;//指向的是根目录，不是文件
    //先把路径找到
    ...
    for(int i=0;i<layers.size();i++){
        ...
        if(layers[i]==".") continue;
            ...
            if(index->getDirName()!=""){ //如果不是根目录，则返回上一级
                index=index->getFather();
            }
            continue;
...
DirNode* findDirByLoc(string& absoluteLocation){
    ...
    for(int i=0;i<layers.size();i++){
        ...
        if(i<layers.size()-1){
            if(layers[i]==".") {
                ...
                if(index->getDirName()!=""){ //如果不是根目录，则返回上一级
                    index=index->getFather();
                    //先去除末尾的“/"
                    newAbsoluteLocation=newAbsoluteLocation.substr(0,newAbsoluteLocation.length()-1);
                    int endIndex= newAbsoluteLocation.rfind("/");
                    if(endIndex!=-1){
                        newAbsoluteLocation=newAbsoluteLocation.substr(0, endIndex+1);
                    }
            ...
            if(layers[i]==".") {
                ...
                if(index->getDirName()!=""){ //如果不是根目录，则返回上一级，并且要把路径中的上一个去除
                    index=index->getFather();
                    //先去除末尾的“/"
                    newAbsoluteLocation=newAbsoluteLocation.substr(0,newAbsoluteLocation.length()-1);
                    int endIndex= newAbsoluteLocation.rfind("/");
        ...
        /*if(index!=NULL){
            cout<<"index="<<index->getDirName()<<endl;
        }
        cout<<"tempPath="<<newAbsoluteLocation<<endl;*/
    }
    newAbsoluteLocation=newAbsoluteLocation.substr(0,newAbsoluteLocation.length()-1);
    absoluteLocation=newAbsoluteLocation;//更新路径
    if(answer==NULL){
...
string searchAllPathInDir(DirNode* node,string fatherPath){
    ...
    for(int i=0;i<subDirs.size();i++){
        outputStr+="\033[31m"+subDirs[i]->getDirName()+"\033[0m  ";
...
string searchAllPathInDirWithNum(DirNode* node,string fatherPath){
    ...
    for(int i=0;i<subDirs.size();i++){
        if(subDirs[i]->getDirName()=="."||subDirs[i]->getDirName()=="..") {
            temp+="\033[31m"+subDirs[i]->getDirName()+"\033[0m\n";
            continue;
        }
        temp+="\033[31m"+subDirs[i]->getDirName()+"\033[0m "+ to_string(subDirs[i]->getNumOfSubDir()-2)+" "+ to_string(subDirs[i]->getNumOfFiles())+"\n"; //子文件目录一定含有“.”和“..”，所以可直接减2
...
// step1:将img文件以二级制的形式读入数组 fileContent中     debug时用另一套
    FILE * imgFile= fopen("/home/nasmenv/Desktop/more/a.img","rb"); //*************  这里要改！！！！！！ 文件名是以“.img”结尾
...
//shep3:不断输入
    while(true){
        ...
        if(command=="exit") break;
            ...
            if(respond=="error"){
                printInNasm("Unrecognized command!\n"); //***************************************************************程序崩溃点2！！！！！！！！！！
                continue;
            }
            File* answer= findFileByLoc(respond);
            if(answer==NULL){
                printInNasm("No such file!\n");
                continue;//程序崩溃点！！！！！！！！！！！！！！！！！！
            }
            string tempAttr=answer->getAttribute();
            if(tempAttr!="00100000"){
                //非普通文件
                if(tempAttr=="00000001"){
                    printInNasm("Read-only file!\n");
                }else if(tempAttr=="00000010"){
                    printInNasm("Hidden File!\n");
                }else if(tempAttr=="00000100"){
                    printInNasm("System file!\n");
                }else if(tempAttr=="00001000"){
                    printInNasm("Volume label\n");
                }
            }
            answer->printFile();
 ...
 *  7.更改过的文件所在文件夹会出现一些看不见的文件，但是属性仍是0x20 名字是“錙UTPU~1”
 */
